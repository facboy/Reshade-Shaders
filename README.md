...
One thing to keep in mind when you make your mask: only the red, green and blue parts of your image are used, the transparency of your mask layer is ignored. If you want the edge of a masked area to fade out instead of stopping abruptly, blur it in your image editor before exporting rather than painting it with partly transparent colors, since a half transparent color would be read the same as the solid one.
...
Now we're going to test your mask. Change UIDM_DIAGNOSTICS back to 0 and you should see your effects turn back on everywhere but the area that you masked. Congratulations, you made a mask and applied it. For the other masks, 2 through 5, you just repeat these steps, entering more float3 and float4 values in UIDetectMulti.fxh. Each time you add a mask you also have to raise UIDM_MASK_COUNT to that mask's number, either in UIDetectMulti.fxh or in the preprocessor definitions section of the reshade menu right next to UIDM_DIAGNOSTICS, because any mask above that number is compiled out and will do nothing.
...
If you cannot find a good pixel for a ui element, all of them change in some way or another, make sure the "Does every pixel needs to be showing to activate?" toggle for that ui element is left unchecked (it is unchecked by default, and each ui element has its own toggle in its mask's Tolerances category) and choose a pixel that changes the least such as a health bar or a weather icon. And take down every value that you see for the colors as they change. Try and get every value that can show up as even a single missed color value can cause the mask to turn off. Once you have all the values go back into UIDetectMulti.fxh and put in as many float3s into the top list with the correct x,y,mask number values as you have colors values. In the bottom list, you're going to need to make a float4 for each and every color value you captured so the shader will look and see if any of them are correct. If one is then the mask turns on. If none are correct numbers then the mask turns off. Remember to raise PIXELNUMBER to the total number of entries in the two lists, counting every ui element you have set up and not just the one you are working on.
