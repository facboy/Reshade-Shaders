...
| `Textures/UIDetectMaskRGBMulti*.png` | Mask images (`UIDetectMaskRGBMulti.png` plus `2`–`5`), 1920x1080, one per mask slot. |
...
- Known staleness: `README.md` still mentions `UIDM_EVERYPIXEL`, which no longer exists — the
  per-element `EveryN` uniforms replaced it. Do not reintroduce that global.
